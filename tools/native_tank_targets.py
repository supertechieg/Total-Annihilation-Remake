...
def main():
    ...
    units = sorted(p.parent.name for p in Path('local/unit-assets').glob('*/unit.json')) if roster else ['armflash', 'corraid', 'armstump', 'armham', 'armpw', 'armrock', 'armwar', 'armsam', 'armjeth']
    for unit in units:
