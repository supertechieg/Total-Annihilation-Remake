"""Original unguided self-propelled update; collision and consolidation isolated."""
import json
import random
...
def main():
    ...
    for index in range(240):
        ...
        native.write(DEFINITION + 0x111, 0x100001)
        ...
        cases.append(dict(tick=tick, deadline=deadline, maximum=maximum, speed=speed, acceleration=acceleration,
                          position=position, velocity=velocity, heading=heading, pitch=pitch, gravity=gravity,
                          expected=dict(position=[signed(native.read(POOL + 4 + axis * 4)) for axis in range(3)],
                                        velocity=[signed(native.read(POOL + 0x1c + axis * 4)) for axis in range(3)], speed=native.read(POOL + 0x3a))))
    Path('local/ballistics/rocket-motion.json').write_text(json.dumps(dict(exe_sha256=EXE_HASH, cases=cases)))
    print(f'NATIVE_ROCKET_MOTION {len(cases)} cases')
