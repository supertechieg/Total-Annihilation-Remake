"""Prepare Comet Catcher's metal grid from original TNT/OTA and feature definitions."""
import hashlib
import json
...
def main():
    ...
    for i in range(width * height):
        ...
        footprint = {zz * width + xx for zz in range(z, z + h) for xx in range(x, x + w)}
    ...
    metadata = dict(version=1, width=width, height=height, surface=surface, features=dict(counts), placements=placements,
    ...
    print(json.dumps({key: metadata[key] for key in ['width', 'height', 'features', 'nonzero_cells', 'values']}, indent=2))
