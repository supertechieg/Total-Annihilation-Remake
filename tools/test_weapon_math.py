...
class WeaponMathTests(unittest.TestCase):
    def test_original_emg_values(self):
        self.assertEqual(weapon_runtime({'weaponvelocity': '300', 'reloadtime': '.4', 'burstrate': '.1'}),
                         dict(velocity_raw_per_tick=655359, reload_ticks=12, burst_interval_ticks=3, minimum_barrel_angle=-0.19634954631328583))
    ...
    def test_missing_values_use_native_defaults(self):
        self.assertEqual(weapon_runtime({}), dict(velocity_raw_per_tick=0, reload_ticks=0, burst_interval_ticks=0,
