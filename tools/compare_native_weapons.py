...
from weapon_math import scaled_weapon_value
...
def main():
    ...
    for item in reference['cases']:
        actual = scaled_weapon_value(item['value'], item['kind'])
    ...
    report = dict(cases=len(reference['cases']), mismatches=len(differences), exe_sha256=reference['exe_sha256'],
                  scope='Weapon velocity, reload and burst-rate text conversion for 193 bundled definitions plus boundary cases; excludes firing schedule and projectile simulation', differences=differences)
