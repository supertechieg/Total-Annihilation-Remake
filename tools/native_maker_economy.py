...
def main():
    ...
    for stock in [0, 30, 60, 120, 1000]:
        ...
        for period in range(16):
            ...
            snapshots.append(dict(income=income, active=active, energy=read(player + 0x8c), metal=read(player + 0x98),
    ...
    Path('local/metal-maker/economy.json').write_text(json.dumps(dict(exe_sha256=EXE_HASH, cases=cases)))
