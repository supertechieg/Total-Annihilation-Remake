...
func _initialize() -> void:
	...
	var index: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(folder.path_join("structures/index.json")))
	var failures := 0
	var total := 0
	var differences: Array = []
	for unit: String in index.units:
		var trace: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(folder.path_join("structures/%s.json" % unit)))
	...
	var report := {"snapshots": total, "mismatches": failures, "units": index.units, "exe_sha256": index.exe_sha256,
		"scope": "Healthy Create and 600 idle ticks with construction finishing at tick 30; excludes damage smoke, SweetSpot/Killed, mine triggering/cloaking and combat", "differences": differences}
	FileAccess.open(folder.path_join("structures/native-comparison.json"), FileAccess.WRITE).store_string(JSON.stringify(report, "  "))
	print("NATIVE_STRUCTURE_COMPARISON %d / %d snapshots match" % [total - failures, total])
