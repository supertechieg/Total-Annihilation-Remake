...
func _initialize() -> void:
	...
	for index in range(trace.cases.size()):
		...
		var actual: Dictionary = JSON.parse_string(JSON.stringify(Launch.solve(case.source, case.target, int(case.speed), int(case.start_speed), int(case.acceleration))))
		if actual != case.expected:
			mismatches.append({"case": index, "actual": actual, "expected": case.expected})
	var report := {"exe_sha256": trace.exe_sha256, "cases": trace.cases.size(), "mismatches": mismatches, "scope": "Original 0x49ca37..0x49cb1c direct launch with supplied start speeds and acceleration; angles, initial speed selection and velocity; excludes acceleration updates, creation and callbacks"}
