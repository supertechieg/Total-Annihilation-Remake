...
const RandomState = preload("res://wind_state.gd")
...
func _initialize() -> void:
	...
	for index in range(trace.cases.size()):
		...
		var random_state := RandomState.new()
		random_state.game_seed = int(case.seed)
		var source := {"position": case.position, "velocity": case.velocity, "heading": int(case.heading), "pitch": int(case.pitch), "timestamp": int(case.timestamp), "deadline": 0, "remaining": int(case.remaining), "removed": false}
		...
		if result.copy != null and int(case.spray) != 0:
			Burst.apply_spread(result.source, int(case.speed), int(case.spray), random_state.bounded_random(int(case.spray)))
		...
		if actual.source != case.source or actual.copy != case.copy or bool(actual.refresh) != (case.queries.size() == 1) or random_state.game_seed != int(case.expected_seed):
		...
		for step: Dictionary in case.get("steps", []):
			...
			transitions += 1
			if actual.source != step.source or actual.copy != step.copy or bool(actual.refresh) != (step.queries.size() == 1) or random_state.game_seed != int(step.expected_seed):
	...
	var report := {"exe_sha256": trace.exe_sha256, "cases": trace.cases.size(), "transitions": transitions, "mismatches": mismatches,
		"scope": "Original 0x49b720 with one burst source: consecutive timing, position refresh, copy/source velocities, native random spread and seed, expiry/removal; controlled muzzle callback; excludes pool consolidation, sounds, duration randomness and capacity exhaustion"}
