...
func _initialize() -> void:
	...
	for case: Dictionary in trace.cases:
		...
		grid.remove_unit(1, rect, int(case.slot), units, inserted, case.yard)
	...
	var report := {"exe_sha256": trace.exe_sha256, "cases": trace.cases.size(), "mismatches": mismatches,
		"scope": "Ordinary and yard-map insert/remove cells, terrain and overlap flags; excludes coarse lists, overlap callbacks and visibility"}
