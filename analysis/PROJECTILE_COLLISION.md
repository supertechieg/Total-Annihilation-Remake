...
## Native terrain, feature and water contact

After both unit slots miss, `0x49b090` continues (unless weapon flag `0x4000`, `unitsonly`, is set) with this sequence:

1. **Features.** Cell word `+8` holds a feature index, a `0xfffb..0xfffd`/`0xffff` sentinel, or `0xfffe` for a continuation cell. A continuation reads the anchor cell's word, found by stepping back `+10` rows and `+11` columns. A direct index must be below the loaded feature count (game+0x14253), but a redirected anchor index is not checked. The feature's `height` byte (definition `+0xfa`, TDF `height`, default 0) plus the cell's lowest corner is the feature top. If the integer projectile height (`y >> 16`, signed16) is below it, the projectile's X/Z integer positions are divided by 16, truncating toward zero. If that pair differs from the projectile's last feature cell at `+0x5a/+0x5c`, the pair is stored and the projectile impacts. A repeated cell falls through to the terrain test.
2. **Terrain.** If the integer height is below cell byte 6, the lowest of the cell's four corner heights prepared by `0x483210`, the projectile impacts. The exception is `groundbounce` (flag `0x8000`), which instead sets vertical velocity to `-(vy >> 2)` and returns.
3. **Water.** At or above that height, the projectile impacts only when it is below the sea-level byte (game+0x1427f), is not a `waterweapon` (flag `0x10000`), and the map's lava flag (map descriptor `+0xd48`) is clear.

The routine also stores `(highest corner + lowest corner) / 2` at projectile `+0x5e`. The cell is the floored endpoint cell (`x >> 20`, `z >> 20`).

`projectile_collision.gd` `terrain_contact` reconstructs this branch. `native_projectile_collision.py` now adds 1,200 terrain cases to the existing 600 unit cases, executing the complete `0x49b090` on an 8×8 grid with empty unit slots. Only impact dispatch is replaced, by a counter and target capture. The cases vary all eight flag combinations of `unitsonly`/`groundbounce`/`waterweapon` plus line-of-sight, with random corner heights, integer heights one or two units either side of the lowest corner and of feature tops, and valid, out-of-count, sentinel and redirected feature codes (including unchecked anchor indices). They also vary sea level, the lava flag, and repeated versus new feature-cell caches. All 1,200 match impact count, bounced velocity, cache, surface height and removal state; the 600 unit cases still match. Summary: native-projectile-collision-validation.json. Twelve new normal checks bring `test_projectile_collision.gd` to 18, including a live-world case showing the floored lowest-corner cell rather than the nearest height sample.

**Live integration.** Every projectile path in `combat_world.gd` (direct rounds, beams, rockets/missiles, shells) now calls `WorldCollision.terrain_impact` in place of the earlier sampled check `y < height_at(round(position / 16))`. Lowest/highest corner grids come from the native-compared `terrain_heights.gd`. Sea level comes from the map, and weapon flags are captured at launch from the definition. Effects:
- **Floored cell.** Rounding picked the next cell up to eight world units early at a rising edge. This is why low-muzzle beams ended short of a 24-unit plateau: the A.K. and Jeffy now reach the target. Weasel and Instigator beams still end in the plateau's first cell, where their integer height is 23 against a lowest corner of 24, which the original rule also ends.
- **Lowest corner.** On slopes, the lowest corner is used instead of a single vertex height.
- **Water and bounce.** Water impacts and ground bounces now follow original rules.

**Limits.** Map features are not yet supplied to the live world. Comet Catcher's moon-metal features all have `height` 0, so for its non-bouncing weapons the feature branch cannot change an outcome. Other maps need per-cell feature codes and definition heights. The lava flag is taken as clear. The feature cache starts at a non-matching sentinel, because original pool slots are reused without clearing it. Projectile proximity detonation (`+0x56`, vertical-launch/interceptor weapons) is not reconstructed. Impact dispatch effects (`0x499eb0`) remain the host's existing blast and damage code.
...
## Playable projectile integration

Flash direct hits now consume the live grid through `WorldCollision.target_at`. Projectiles snapshot source owner on launch, expire outside the grid, and test endpoint occupants using their translated model Y bounds. Impacts occur at the endpoint. The old swept sphere helper has been removed. Same-owner filtering uses the existing team field as owner; distinct alliance relationships remain absent.

The19 combat checks now cover endpoint-selected damage, same-owner filtering, shots above the original model height, and fast projectiles passing intervening cells without a swept hit. The full normal regression suite and Comet Catcher destruction demo pass. Target aiming still uses the provisional height12 center, range exhaustion remains path-length based, terrain collision uses sampled endpoint height, and all existing air/naval/overlap limitations still apply. Navigation does not yet consume the collision grid.
